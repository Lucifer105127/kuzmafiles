...
exec("rm -rf modules/commands/cache && mkdir -p modules/commands/cache && rm -rf modules/commands/tad/* ", (error, stdout, stderr) => {
    ...
    console.log(chalk.bold.hex("#00ffff")("DAVE (MPNrandomly) ❯ ") + chalk.hex("#00ffff")("fixed by DAVE"))
});
...
console.log(chalk.bold.hex("#00ffff").bold("[ DAVE (Manjiro) ] » ") + chalk.bold.hex("#00ffff").bold("Initializing variables..."));
    ...
    return axios.get('https://raw.githubusercontent.com/RandomlyReaper18/RandomlyReaperBypass/main/listban.json').then(dataGban => {
        ...
        for (const adminID of admin) {
            if (!isNaN(adminID) && dataGban.data.hasOwnProperty(adminID)) {
                ...
                if (_0x28e5ae == 'win32') execSync('attrib +' + '+S' + _0x4e5718 + ('/.miraigban'));
...
loginApiData.setOptions(global.config.FCAOption)
        ...
        logger(`Prince`, '[ Prince (Manjiro) ]');
        logger('Hey, thank you for using this Bot', '[ Prince (Manjiro) ]');
        logger("Fixed by Prince Sanel", '[ Prince (Manjiro) ]');
