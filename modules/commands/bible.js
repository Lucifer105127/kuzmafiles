...
var callback = () => api.sendMessage({body:`${res.data[0].bookname} ${res.data[0].chapter}:${res.data[0].verse}\n${res.data[0].text}` ,attachment: fs.createReadStream(__dirname + "/cache/mahiro.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/mahiro.jpg")); 
      return request(encodeURI(link[Math.floor(Math.random() * link.length)])).pipe(fs.createWriteStream(__dirname+"/cache/mahiro.jpg")).on("close",() => callback());
} catch (error) {
