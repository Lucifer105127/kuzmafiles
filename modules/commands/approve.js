module.exports.config = {
	...
	hasPermssion: 2,
	credits: "Matthew",
	description: "approve the gc using bots lmao",
...
module.exports.run = async ({ event, api, args, Threads, handleReply, Users }) => {
  ...
  const cre = this.config.credits
      ...
      api.sendMessage(`[ OKIE ] Your group has been removed from the browsing list by the admin for the reason: ${lydo}`, idBox);
    	api.sendMessage(`[ OKIE ] Box has been removed from the list of allowed bots`, threadID, () => {
   		...
   		api.changeNickname(`${global.config.BOTNAME}`, idBox, global.data.botID);
      const axios = require('axios');
	const request = require('request');
	const fs = require("fs");
   let admID = "100077357343941";    
